  module Tpl
    ...
    Tpl_Fun = Function
        ...
        Tokens = List
         ...
         @Uniontype Text begin
              ...
              @Record FILE_TEXT begin
                       ...
                       opaqueFile::Option{ModelicaInteger}
                       nchars::Array{ModelicaInteger}
                       aind::Array{ModelicaInteger}
                       isstart::Array{Bool}
                       blocksStack::Array{List{BlockTypeFileText}}
              end
         end
         ...
         @Uniontype BlockTypeFileText begin
              @Record BT_FILE_TEXT begin
                       ...
                       isstart::Bool
                       tell #= Usage depends on bt; stores the last file position to know if it is empty or not. =#::Array{ModelicaInteger}
                       septok::Array{Option{StringToken}}
              end
         end
         ...
         @Uniontype IterOptions begin
              @Record ITER_OPTIONS begin
                       ...
                       empty::Option{StringToken}
                       separator::Option{StringToken}
              ...
              txt = begin
                ...
                @match txt begin
                  ...
                  FILE_TEXT(__)  => begin
                       ...
                       #= /* as (_::_) */ =#
                      _ = begin
                        @match listGet(arrayGet(txt.blocksStack, 1), 1) begin
                          BT_FILE_TEXT(bt = BT_ITER(options = iopts, index0 = i0), tell = tell, septok = septok)  => begin
                               #=  Either the iterator always increments, or the file position changed
                               =#
                              tellpos = textFileTell(txt)
                              if arrayGet(tell, 1) != tellpos
                                arrayUpdate(tell, 1, tellpos)
                                txt2 = txt
                                haveToken = true
                              else
                                txt2 = begin
                                  @match iopts.empty begin
                                    NONE()  => begin
        ...
        function tplCallWithFailError(inFun::Tpl_Fun, inArg::ArgType1, txt::Text = emptyTxt) ::Text
              ...
              txt = tplCallHandleErrors((inArg) -> inFun(inArgA = inArg), txt)
          txt
        end
        ...
        function tplCallWithFailError2(inFun::Tpl_Fun, inArgA::ArgType1, inArgB::ArgType2, txt::Text = emptyTxt) ::Text
              ...
              txt = tplCallHandleErrors((inArgA, inArgB) -> inFun(inArgA = inArgA, inArgB = inArgB), txt)
          txt
        end
