    ...
    Fun = Function
        ...
        function simplifyRSub(e::DAE.Exp) ::DAE.Exp 
              ...
              e = begin
                ...
                @match e begin
                  DAE.RSUB(exp = DAE.CREF(componentRef = cr), ix = #= AbsynDumpTpl.dumpPattern: UNHANDLED Abyn.Exp  =#)  => begin
                    DAE.CREF(ComponentReference.joinCrefs(cr, ComponentReference.makeCrefIdent(e.fieldName, e.ty, nil)), e.ty)
                  end
                  
                  DAE.RSUB(exp = DAE.CALL(path = p1, expLst = exps, attr = DAE.CALL_ATTR(ty = DAE.T_COMPLEX(complexClassType = ClassInf.RECORD(path = p2), varLst = vars))), ix = #= AbsynDumpTpl.dumpPattern: UNHANDLED Abyn.Exp  =#) where (AbsynUtil.pathEqual(p1, p2))  => begin
                    listGet(exps, ListUtil.position1OnTrue(list(v.name for v in vars), stringEq, e.fieldName))
                  end
                  
                  DAE.RSUB(exp = DAE.RECORD(exps = exps, comp = comp), ix = #= AbsynDumpTpl.dumpPattern: UNHANDLED Abyn.Exp  =#)  => begin
        ...
        function simplifyRangeReal2(inStart::ModelicaReal, inStep::ModelicaReal, inSteps::ModelicaInteger, inValues::List{<:ModelicaReal}) ::List{ModelicaReal} 
              ...
              outValues = begin
                ...
                @match (inStart, inStep, inSteps, inValues) begin
                  (_, _, #= AbsynDumpTpl.dumpPattern: UNHANDLED Abyn.Exp  =#, _)  => begin
                    inValues
                  end
