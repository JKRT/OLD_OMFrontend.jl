                ...
                outComponentRef = begin
                  ...
                  @matchcontinue inComponentRef begin
                    ...
                    _  => begin
                        ...
                        print("CREF_LOCAL.unelabCref failed on: " + printComponentRefStr(inComponentRef) + "\\n")
                      fail()
                    end
                  end
                end
        ...
        module CompareWithSubsType
         ...
         function crefLastSubs(inComponentRef::DAE.ComponentRef) ::List{DAE.Subscript}
          ...
          function crefTypeFull2(inCref::DAE.ComponentRef, accumDims::List{<:DAE.Dimension} = nil) ::Tuple{DAE.Type, List{DAE.Dimension}}
                ...
                (outType, outDims) = begin
                  ...
                  @match inCref begin
                    ...
                    _  => begin
                          @match true = Flags.isSet(Flags.FAILTRACE)
                          Debug.trace("CREF_LOCAL.crefTypeFull2 failed on cref: ")
          ...
          function crefType(inCref::DAE.ComponentRef) ::DAE.Type
                ...
                outType = begin
                  ...
                  @match inCref begin
                    ...
                    _  => begin
                          @match true = Flags.isSet(Flags.FAILTRACE)
                          Debug.trace("CREF_LOCAL.crefType failed on cref: ")
                          Debug.traceln(printComponentRefStr(inCref))
                        fail()
                    end
                  end
          ...
          function crefTypeConsiderSubs(cr::DAE.ComponentRef) ::DAE.Type
           ...
           #= Function: crefType
          Function for extracting the name and type out of the first cref of a CREF_LOCAL.
           =#
          ...
          function crefApplySubs(inComponentRef::DAE.ComponentRef, inSubs::List{<:DAE.Subscript}) ::DAE.ComponentRef
                ...
                outComponentRef = begin
                  ...
                  @match inComponentRef begin
                    ...
                    _  => begin
                          Error.addInternalError("function CREF_LOCAL.crefApplySubs to non array\\n", sourceInfo())
                        fail()
                    end
                  end
                end
          ...
          function expandCref(inCref::DAE.ComponentRef, expandRecord::Bool) ::List{DAE.ComponentRef}
                ...
                outCref = begin
                  @matchcontinue (inCref, expandRecord) begin
                    ...
                    _  => begin
                          @match true = Flags.isSet(Flags.FAILTRACE)
                          Debug.traceln("- CREF_LOCAL.expandCref failed on " + printComponentRefStr(inCref))
                        fail()
                    end
                  end
                end
          ...
          function createDifferentiatedCrefName(inCref::DAE.ComponentRef, inX::DAE.ComponentRef, inMatrixName::String) ::DAE.ComponentRef
                ...
                if debug
                  print("inCref: " + CREF_LOCAL.printComponentRefStr(inCref) + "\\n")
                end
                 #=  move subs and and type to lastCref, to move type replace by last type
                 =#
                 #=  and move last cref type to the last cref.
                 =#
                subs = CREF_LOCAL.crefLastSubs(inCref)
                outCref = CREF_LOCAL.crefStripLastSubs(inCref)
                outCref = CREF_LOCAL.replaceSubsWithString(outCref)
                if debug
                  print("after full type  " + Types.printTypeStr(CREF_LOCAL.crefTypeConsiderSubs(inCref)) + "\\n")
                end
                outCref = CREF_LOCAL.crefSetLastType(outCref, DAE.T_UNKNOWN_DEFAULT)
                if debug
                  print("after strip: " + CREF_LOCAL.printComponentRefListStr(CREF_LOCAL.expandCref(outCref, true)) + "\\n")
                end
                 #=  join crefs
                 =#
                outCref = CREF_LOCAL.joinCrefs(outCref, CREF_LOCAL.makeCrefIdent(DAE.partialDerivativeNamePrefix + inMatrixName, DAE.T_UNKNOWN_DEFAULT, nil))
                outCref = CREF_LOCAL.joinCrefs(outCref, inX)
                if debug
                  print("after join: " + CREF_LOCAL.printComponentRefListStr(CREF_LOCAL.expandCref(outCref, true)) + "\\n")
                end
                 #=  fix subs and type of the last cref
                 =#
                outCref = CREF_LOCAL.crefSetLastSubs(outCref, subs)
                outCref = CREF_LOCAL.crefSetLastType(outCref, CREF_LOCAL.crefLastType(inCref))
                if debug
                  print("outCref: " + CREF_LOCAL.printComponentRefStr(outCref) + "\\n")
                end
            outCref
          end
...
######################################
              ...
              outExp = begin
                ...
                @match inExp begin
                  ...
                  DAE.CREF(cr, ty)  => begin
                      crs = CREF_LOCAL.stringifyComponentRef(cr)
                    makeCrefExp(crs, ty)
                  end
        ...
        function expStripLastSubs(inExp::DAE.Exp) ::DAE.Exp
              ...
              outExp = begin
                ...
                @match inExp begin
                  DAE.CREF(componentRef = cr)  => begin
                      ty = CREF_LOCAL.crefLastType(cr)
                      cr_1 = CREF_LOCAL.crefStripLastSubs(cr)
        ...
        function expStripLastIdent(inExp::DAE.Exp) ::DAE.Exp
              ...
              outExp = begin
                ...
                @match inExp begin
                  DAE.CREF(componentRef = cr)  => begin
                      cr_1 = CREF_LOCAL.crefStripLastIdent(cr)
                      ty = CREF_LOCAL.crefLastType(cr_1)
                      e = makeCrefExp(cr_1, ty)
                    e
                  end
              ...
              outExp = begin
                ...
                @match (exp, subscr) begin
                  (DAE.CREF(cr, t), _)  => begin
                      cr1 = CREF_LOCAL.crefStripLastSubs(cr)
                      subs = CREF_LOCAL.crefLastSubs(cr)
                      cr2 = CREF_LOCAL.subscriptCref(cr1, _cons(subscr, subs))
                      e = makeCrefExp(cr2, t)
                    e
                  end
                end
              ...
              outArg = begin
                ...
                @match (inExp, inSubs) begin
                  ...
                  (DAE.CREF(cref, ty), _)  => begin
                      cref = CREF_LOCAL.subscriptCref(cref, inSubs)
                    DAE.CREF(cref, ty)
                  end
        ...
        function getAllCrefs2(inExp::DAE.Exp, inCrefList::List{<:DAE.ComponentRef}) ::Tuple{DAE.Exp, List{DAE.ComponentRef}}
              ...
              if isCref(inExp)
                ...
                if ! CREF_LOCAL.crefEqual(cr, DAE.crefTime) && ! listMember(cr, inCrefList)
        ...
        function getAllCrefsExpanded2(inExp::DAE.Exp, inCrefList::List{<:DAE.ComponentRef}) ::Tuple{DAE.Exp, List{DAE.ComponentRef}}
              ...
              if isCref(inExp)
                @match DAE.CREF(componentRef = cr) = inExp
                crlst = CREF_LOCAL.expandCref(cr, true)
                for c in crlst
                  if ! CREF_LOCAL.crefEqual(c, DAE.crefTime) && ! listMember(c, inCrefList)
                ...
                @match (inCref, inExpType) begin
                  (cref, tGiven)  => begin
                      if Flags.isSet(Flags.CHECK_DAE_CREF_TYPE)
                        tExisting = CREF_LOCAL.crefLastType(cref)
                        if ! valueEq(tGiven, tExisting)
                          Debug.traceln("Warning: Expression.makeCrefExp: cref " + ExpressionDump.printComponentRefStr(cref) + " was given type DAE.CREF.ty: " + Types.unparseType(tGiven) + " is different from existing DAE.CREF.componentRef.ty: " + Types.unparseType(tExisting))
                        end
                      end
        ...
        function crefToExp(cr::DAE.ComponentRef) ::DAE.Exp
              ...
              cref = DAE.CREF(cr, CREF_LOCAL.crefTypeFull(cr))
          cref
        end
        ...
        function crefExp(cr::DAE.ComponentRef) ::DAE.Exp
              ...
              cref = begin
                ...
                @match cr begin
                  _  => begin
                      ty1 = CREF_LOCAL.crefLastType(cr)
                      cref = begin
                        @match ty1 begin
                          DAE.T_ARRAY(__)  => begin
                              subs = CREF_LOCAL.crefLastSubs(cr)
              ...
              outCrefExp = begin
                ...
                @match (inVar, inCrefPrefix) begin
                  (DAE.TYPES_VAR(name = name, ty = ty), _)  => begin
                      cr = CREF_LOCAL.crefPrependIdent(inCrefPrefix, name, nil, ty)
                      e = makeCrefExp(cr, ty)
                    e
                  end
                end
        ...
        function generateCrefsExpFromExp(inExp::DAE.Exp, inCrefPrefix::DAE.ComponentRef) ::DAE.Exp
              ...
              outCrefExp = begin
                ...
                @match inExp begin
                  ...
                  DAE.CREF(componentRef = cr, ty = ty)  => begin
                      name = CREF_LOCAL.crefModelicaStr(cr)
                      cr = CREF_LOCAL.crefPrependIdent(inCrefPrefix, name, nil, ty)
                      e = makeCrefExp(cr, ty)
                    e
                  end
              ...
              outCrefExpList = begin
                ...
                @match (inExp, inCrefPrefix) begin
                  ...
                  (DAE.CALL(path = Absyn.IDENT("der"), expLst = DAE.CREF(componentRef = incref) <|  nil()), _)  => begin
                      cr = CREF_LOCAL.crefPrefixDer(incref)
                  ...
                  (DAE.CREF(componentRef = cr, ty = ty), SOME(incref))  => begin
                      name = CREF_LOCAL.crefModelicaStr(cr)
                      cr = CREF_LOCAL.crefPrependIdent(incref, name, nil, ty)
                      e = makeCrefExp(cr, ty)
                    list(e)
                  end
        ...
        function replaceCref(inExp::DAE.Exp, inTpl::Tuple{<:DAE.ComponentRef, DAE.Exp}) ::Tuple{DAE.Exp, Tuple{DAE.ComponentRef, DAE.Exp}}
              ...
              (outExp, otpl) = begin
                  ...
                  local cr1::DAE.ComponentRef
                @match (inExp, inTpl) begin
                  (DAE.CREF(componentRef = cr), (cr1, target)) where (CREF_LOCAL.crefEqualNoStringCompare(cr, cr1))  => begin
                    (target, inTpl)
                  end
        ...
        function extractUniqueCrefsFromExp(inExp::DAE.Exp) ::List{DAE.ComponentRef}
              local ocrefs::List{DAE.ComponentRef}
               ...
               #=  ocrefs := List.unique(List.flatten(List.map1(extractCrefsFromExp(inExp), CREF_LOCAL.expandCref, true)));
               =#
              ...
              (outExp, crefs) = begin
                ...
                @match (inExp, inCrefs) begin
                  (DAE.CREF(componentRef = cr), crefs)  => begin
                      crefs = ListUtil.unionEltOnTrue(cr, crefs, CREF_LOCAL.crefEqual)
                    (inExp, crefs)
                  end
              ...
              outExp = begin
                ...
                @matchcontinue inExp begin
                  ...
                  (e && DAE.CALL(path = Absyn.IDENT(name = "der"), expLst = DAE.CREF(cr, _) <|  nil()), (crefs, dcrefs))  => begin
                      dcrefs = ListUtil.unionEltOnTrue(cr, dcrefs, CREF_LOCAL.crefEqual)
                    (e, (crefs, dcrefs))
                  end
              ...
              (outExp, cont, outTpl) = begin
                ...
                @matchcontinue (inExp, inTpl) begin
                  (DAE.CREF(componentRef = cr1), (cr, false))  => begin
                      b = CREF_LOCAL.crefEqualNoStringCompare(cr, cr1)
                    (inExp, ! b, if b
                          (cr, b)
                        else
                          inTpl
              ...
              (outExp, cont, outTpl) = begin
                ...
                @match (inExp, inTpl) begin
                  (DAE.CREF(componentRef = cr), (name, false))  => begin
                      b = name == CREF_LOCAL.crefFirstIdent(cr)
                    (inExp, ! b, if b
                          (name, b)
                        else
                          inTpl
        ...
        function traversingexpHasDerCref(inExp::DAE.Exp, inTpl::Tuple{<:DAE.ComponentRef, Bool}) ::Tuple{DAE.Exp, Bool, Tuple{DAE.ComponentRef, Bool}}
              ...
              (outExp, cont, outTpl) = begin
                ...
                @matchcontinue (inExp, inTpl) begin
                  ...
                  (DAE.CALL(path = Absyn.IDENT("der"), expLst = DAE.CREF(componentRef = cr1) <|  nil()), (cr, false))  => begin
                      b = CREF_LOCAL.crefPrefixOf(cr, cr1)
                    (inExp, ! b, if b
                          (cr, b)
                        else
                          inTpl
        ...
        function traversingexpHasDer(inExp::DAE.Exp, inTpl::Bool) ::Tuple{DAE.Exp, Bool, Bool}
              ...
              (outExp, cont, outTpl) = begin
                ...
                @match (inExp, inTpl) begin
                  ...
                  (DAE.CREF(componentRef = cr), false) where (intEq(System.strncmp(CREF_LOCAL.crefFirstIdent(cr), "DERAlias", 9), 0))  => begin
                    (inExp, false, true)
                  end
              ...
              (outExp, cont, outTpl) = begin
               ...
               #= /* Not reachable...
                  case (DAE.CREF(componentRef = cr1), (cr,false))
                    equation
                      b = CREF_LOCAL.crefPrefixOf(cr1,cr);
              ...
              outIsCall = begin
                ...
                @match (inExp, funcsIn) begin
                  (DAE.CALL(path = path), _)  => begin
                    ...
                    listEmpty(getFunctionElements(func))
                  end
        ...
        function isCrefScalar(inExp::DAE.Exp) ::Bool
              ...
              isScalar = begin
                ...
                @matchcontinue inExp begin
                  DAE.CREF(ty = DAE.T_ARRAY(__))  => begin
                      cr = expCref(inExp)
                      b = CREF_LOCAL.crefHasScalarSubscripts(cr)
                    b
                  end
              ...
              outBoolean = begin
                ...
                @matchcontinue (inExp1, inExp2) begin
                  ...
                  (DAE.CREF(componentRef = cr1), DAE.CREF(componentRef = cr2))  => begin
                      res = CREF_LOCAL.crefEqual(cr1, cr2)
                      if ! res
                        expLst = ListUtil.map(CREF_LOCAL.crefSubs(cr1), getSubscriptExp)
                        res = ListUtil.map1BoolOr(expLst, expContains, inExp2)
                      end
                    res
                  end
        ...
        function splitRecord2(var::DAE.Var, cr::DAE.ComponentRef) ::DAE.Exp
              ...
              exp = makeCrefExp(CREF_LOCAL.crefPrependIdent(cr, n, nil, ty), ty)
          exp
        end
        ...
        function arrayElements(inExp::DAE.Exp) ::List{DAE.Exp}
              ...
              outExp = begin
                ...
                @match inExp begin
                  DAE.CREF(componentRef = cr)  => begin
                      crl = CREF_LOCAL.expandCref(cr, false)
              ...
              (outExp, outCr) = begin
                ...
                @matchcontinue (e, optCr) begin
                  (DAE.CALL(path = Absyn.IDENT("der"), expLst = DAE.CREF(componentRef = cr) <|  nil()), SOME(cref))  => begin
                      derCr = CREF_LOCAL.crefPrefixDer(cr)
                      @match true = CREF_LOCAL.crefEqualNoStringCompare(derCr, cref)
                      cref_exp = crefExp(derCr)
                    (cref_exp, optCr)
                  end

                  (DAE.CALL(path = Absyn.IDENT("der"), expLst = DAE.CREF(componentRef = cr) <|  nil()), NONE())  => begin
                      cr = CREF_LOCAL.crefPrefixDer(cr)
                      cref_exp = crefExp(cr)
                    (cref_exp, NONE())
                  end
              ...
              oCrefWithEqualIdents = begin
                @matchcontinue iExpressions begin
                  head <| _  => begin
                      ...
                      tmpCrefWithEqualIdents = ListUtil.map1BoolAnd(crefs, CREF_LOCAL.crefEqualWithoutLastSubs, headCref)
              ...
              exp = begin
                ...
                @match exp begin
                  DAE.CREF(ty = DAE.T_ARRAY(ty = ty))  => begin
                    makeArray(list(makeCrefExp(cr, ty) for cr in CREF_LOCAL.expandCref(exp.componentRef, true)), exp.ty, ! Types.isArray(ty))
                  end
        ...
        function expandExpression(inExp::DAE.Exp, expandRecord::Bool) ::List{DAE.Exp}
              ...
              outExps = begin
                ...
                @match inExp begin
                  DAE.CREF(cr, _)  => begin
                      crlst = CREF_LOCAL.expandCref(cr, expandRecord)
              ...
              comp = begin
                ...
                @match inExp1 begin
                  ...
                  DAE.CREF(__)  => begin
                    ...
                    CREF_LOCAL.crefCompareGeneric(inExp1.componentRef, cr)
                  end
                  ...
                  DAE.EMPTY(__)  => begin
                    ...
                    CREF_LOCAL.crefCompareGeneric(inExp1.name, cr)
                  end
